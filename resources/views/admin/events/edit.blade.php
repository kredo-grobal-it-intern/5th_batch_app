        ...
        <div class="py-4 bg-white rounded">
            <form action="{{ route('admin.events.update', ['event' => $event->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex px-6 pb-4 border-b">
                    ...
                    <div class="ml-auto">
                        <button type="submit" class="py-2 px-3 text-xs text-white font-semibold bg-indigo-500 rounded-md">Update</button>
                    </div>
                </div>
                ...
                <div class="pt-4 px-6">
                    ...
                    <div class="mb-6">
                        ...
                        <input id="title" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded" type="text" name="title" value="{{ old('title', $event->title) }}">
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium mb-2" for="image">Image</label>
                        <div class="flex items-end">
                            <img id="previewImage" src="{{ asset('storage/'. $event->image) }}" data-noimage="{{ asset('storage/'. $event->image) }}" alt="" class="rounded shadow-md w-64">
                            <input id="image" class="block w-full px-4 py-3 mb-2" type="file" accept='image/*' name="image">
                        </div>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium mb-2" for="body">Body</label>
                        <textarea id="body" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded" name="body" rows="5">{{ old('body', $event->body) }}</textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium mb-2">Dogs appearing in</label>
