...
class NewsController extends Controller
{
    ...
    public function showAmusement(){
        ...
        return view('useful-info.articles.all-articles.amusement')->with('amusement_news', $amusement_news);
        }

    public function showCafe(){
        $cafe_news = News::latest()->where('news_type', 'pet cafe')->paginate(8);

        return view('useful-info.articles.all-articles.cafe')->with('cafe_news', $cafe_news);
    }

    public function showDogrun(){
        $dogrun_news = News::latest()->where('news_type', 'dogrun')->paginate(8);

        return view('useful-info.articles.all-articles.dogrun')->with('dogrun_news', $dogrun_news);
    }

    public function showHospital(){
        $hospital_news = News::latest()->where('news_type', 'pet hospital')->paginate(8);

        return view('useful-info.articles.all-articles.hospital')->with('hospital_news', $hospital_news);
    }
